...
def parse_args():
    ...
    parser.add_argument('--repo_id', type=str, default='detr-resnet-50-finetuned-600-epochs-Kaohsiung-Port-dataset', help='Repository ID')
    parser.add_argument('--dataset_id', type=str, default='Kaohsiung_Port_dataset_2024', help='Dataset ID')
...
def main():
    ...
    dataset = load_dataset(f"{args.hub_id}/{args.dataset_id}")
    ...
    ############# Preprocessing #############
    checkpoint_resume = f"{args.hub_id}/{args.repo_id}"
    ...
    def transform_aug_ann(examples):
        ...
        for image_path, objects in zip(examples["image_path"], examples["objects"]):
            try:
                ...
                image = np.array(image)[:, :, ::-1]  # Convert to OpenCV format (BGR)
            
            except (FileNotFoundError, UnidentifiedImageError) as e:
                print(f"Error: {e}")
                image = create_empty_image()  # Use a black placeholder image
    ...
    training_args = TrainingArguments(
        ...
        push_to_hub=True,
        hub_model_id=f"{args.hub_id}/{args.repo_id}",
    )
    ...
    trainer.push_to_hub(commit_message=f'detr-resnet-50-finetuned-boat-dataset-{args.epoch}-epochs')
